...
/** Descriptive image metadata retained by the canonical sampled value.
  *
  * Metadata does not participate in grid identity or sampling compatibility. The empty value is
  * shared so unlabeled images do not require a distinct metadata allocation.
  */
